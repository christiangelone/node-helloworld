  ...
  try {
    ...
    stage("Success") {
      ...
      slackSend channel: '#jenkins-builds',
                color: 'good',
                message: "(${currentBuild.duration} ms) Pipeline ${currentBuild.fullDisplayName} from branch ${BRANCH_NAME}: Build has completed successfully (App tested & deployed)"
    }${env.BRANCH_NAME}
  } catch(err) {
    ...
    stage('Failure') {
      ...
      slackSend channel: '#jenkins-builds',
                color: 'danger',
                message: "(${currentBuild.duration} ms) Pipeline ${currentBuild.fullDisplayName} from branch ${BRANCH_NAME}: Build has failed"    
    }
  } finally {
    stage('Finished') {
